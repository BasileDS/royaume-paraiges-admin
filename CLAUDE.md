...
- **`<BottomSheet open onOpenChange title description? footer? hideHeader? />`** (`src/components/ui/bottom-sheet.tsx`, septembre 2026) : feuille qui **glisse depuis le bas** sous `md` (poignée, glisser vers le bas pour fermer, corps défilant, marge de sécurité `env(safe-area-inset-bottom)`), **dialog centré** au-delà. Même primitive Radix Dialog que `<Dialog>` (focus, échap, overlay, verrou du scroll) ; sur téléphone l'auto-focus est neutralisé pour ne pas faire surgir le clavier à l'ouverture. À préférer à `<Dialog>` pour tout formulaire court ou menu d'actions qui doit s'utiliser au pouce. Pour garder le dernier élément affiché pendant l'animation de fermeture, dériver un state en rendu (`if (item !== null && item !== displayed) setDisplayed(item)`), jamais un effet : voir `menus/[id]/_components/menu-item-sheet.tsx`.
...
**`/links`** — Liens de redirection (juillet 2026, migration **063**) : gestion des liens courts `redirects.auxparaiges.fr/<slug>` servis par le projet frère `url-rooting-app` (Vercel, 302 + log des clics). Tables `redirect_links` / `redirect_clicks` + vue `redirect_link_stats` (RLS admin-only ; l'app de redirection écrit en service_role). Listing `<DataTable>` (copie d'URL, badge Smart link si cibles iOS/Android, toggle actif inline, compteur + dernier clic via la vue) ; formulaire partagé création/édition `links/_form/LinkForm.tsx` (RHF + Zod `redirectLink.schema.ts`, section repliable smart links, erreur 23505 → « slug déjà utilisé ») ; détail `links/[id]` = form + `link-qr-card.tsx` (QRCodeSVG de l'URL courte, téléchargement SVG et PNG 1024px via canvas — le QR encode l'URL courte, jamais la cible) + `link-stats.tsx` (compteurs total/7j/30j, BarChart Recharts empilé par appareil sur 30 jours, fetch paginé par 1000 de `redirect_clicks`). Service `redirectLinkService.ts` (`REDIRECT_BASE_URL`, `buildShortUrl`), keys `redirectLinkKeys`, feature key `links` (groupe Contenu). Suppression = cascade sur l'historique (ConfirmDialog le rappelle ; préférer la désactivation, qui redirige vers le fallback auxparaiges.fr).

**`/menus`** — Cartes & menus (septembre 2026, migrations **094 à 101**) : administration des cartes des établissements, reprises de l'application `menu-ripaille`. **Réservée au rôle `admin`** : un gérant (`establishment`) n'administre pas sa carte, il n'existe donc aucune table d'appartenances.

**Un admin ne modifie que les cartes de son établissement de rattachement et de son groupe (migrations 109 + 113, septembre 2026).** Le périmètre est `profiles.attached_establishment_id`, étendu aux établissements du même `establishments.group_id` (113 : un admin d'Aux Paraiges édite aussi le Garage et la Grange, groupe « Les Paraiges (Metz) ») : un admin non super écrit sur ces seules cartes, un super admin sur toutes, la **lecture reste ouverte** à tout admin (les autres cartes s'ouvrent en lecture seule). Appliqué **en RLS** sur les 13 tables `menu_*` par le helper `admin_can_edit_menu(establishment_id)` (= `admin_has_feature('menus')` puis super admin, rattachement ou même groupe) : colonne `establishment_id` sur les tables qui en ont une, remontée au parent pour les filles (variantes, options, paliers, pivot item ↔ groupe), et **écriture super admin seulement** sur les référentiels partagés (`menu_item_types`, `menu_catalog_products`, `menu_events`), puisqu'un soft du catalogue s'affiche sur plusieurs cartes. Côté UI, `menus/_lib/access.ts` (`useMenuAccess`) lit le périmètre **en base** via la RPC `admin_editable_menu_establishments()` (query key `menuKeys.editable()`, la règle n'est pas recalculée côté client) et pilote le mode lecture seule de `/menus/[id]` (bandeau, prop `readOnly` des lignes et en-têtes, barre d'actions et boutons de création retirés) et `menus/_components/menu-edit-guard.tsx` barre les formulaires produit ; la liste `/menus` met l'établissement de rattachement en tête et badge les autres « lecture seule ». ⚠️ Sous RLS, un UPDATE/DELETE hors périmètre ne renvoie **pas d'erreur mais zéro ligne** : `menuService` relit les lignes touchées (`assertTouched` → `42501 MENU_WRITE_IGNORED`) et la RPC `set_menu_item_featured` lève `42501 MENU_EDIT_FORBIDDEN`, les deux traduits par `describeMenuError`. Le bucket `menu-assets` n'est pas scopé (aucun écran d'upload à date).

Deux pages. **`/menus`** affiche **une carte par établissement** (grille, septembre 2026 - plus de `<DataTable>`) avec l'état de sa carte (carte vide, hors carte / rupture, happy hour, état du lien court - **sans compteurs** catégories / produits / bières, retirés à la demande le 09/09/2026) via `getEstablishmentMenuSummaries` — trois requêtes à plat agrégées côté client plutôt qu'une RPC de plus. Toute la carte ouvre l'édition (`/menus/[id]`, lien étiré `after:absolute after:inset-0`) ; deux icônes dans l'en-tête passent au-dessus (`relative z-10`) : **ouvrir la carte publique** `menus.auxparaiges.fr/<slug>` dans un nouvel onglet (`buildPublicMenuUrl`, désactivée tant qu'aucune catégorie ni produit n'est saisi) et **voir / télécharger le QR code** (`_components/menu-qr-sheet.tsx`, `<BottomSheet>`), qui réutilise `src/components/link-qr-code.tsx` — le QR + exports SVG/PNG extraits de `links/[id]/_components/link-qr-card.tsx`, les deux pages rendent donc strictement le même code. ⚠️ **Aucune FK entre `redirect_links` et `establishments`** : `findMenuRedirectLink(links, slug)` (`menuService`) rapproche le lien court par sa **cible par défaut** (`target_url` = `menus.auxparaiges.fr/<slug>`, casse et `/` final ignorés) ; les liens viennent de `getRedirectLinks` sous `redirectLinkKeys.lists()`, donc toute mutation dans `/links` rafraîchit aussi `/menus`. Sans lien correspondant, l'icône QR est désactivée (tooltip) ; lien inactif = pastille ambre et avertissement dans la feuille (le QR renvoie vers `auxparaiges.fr`), lien vers `/links/[id]` en pied de feuille pour l'activer. Le logo (`establishments.logo`, bucket `content-assets` via `getImageUrl`) illustre la carte, initiale en repli. **`/menus/[id]`** rend la carte telle que la voit le client : catégories sur deux niveaux, items avec leurs formats tarifés, et une section « Disponibles, hors carte » pour les items sans catégorie.

**Trois notions à ne pas confondre**, elles sont distinctes en base et l'UI doit le rester : `category_id IS NULL` = disponible mais **hors carte affichée** (c'est ce qui garde une bière visible dans l'app Compagnon) ; `is_active = false` = à la carte mais **en rupture** ; absence de ligne = **pas à la carte du tout**. Le bouton œil barré fait le premier, l'interrupteur le second. Les 43 liaisons « disponible hors carte » héritées de l'ancienne `beers_establishments` (reprise 095) ont été **supprimées par la migration 111** (09/09/2026) : l'app des Compagnons n'affiche pas la disponibilité par établissement (composants hors navigation), la carte est la seule source. La section « Disponibles, hors carte » reste, vide par défaut, et n'y affiche plus l'alerte « invisible sur la carte publique », redondante pour un item sans catégorie.
...
**Chapitres (migration 107, septembre 2026)** : une catégorie **racine** peut rejoindre un chapitre (`menu_sections`, colonne `menu_categories.section_id`), demandé pour rassembler les six familles de cocktails du Garage. Un chapitre n'a **ni produit, ni position, ni interrupteur** : il prend la place de sa première catégorie visible et disparaît avec la dernière, sur la seule échelle de tri qui existe, celle des catégories. Il se crée depuis la fiche d'une catégorie (« Nouveau chapitre… » dans le sélecteur Chapitre, créé avant la catégorie), se modifie et se dissout depuis son en-tête (`section-group.tsx`, `section-actions-sheet.tsx`, `section-dialog.tsx`) ; dissoudre remet les catégories à plat (FK en SET NULL). Le sélecteur se désactive dès qu'un parent est choisi : le trigger `trg_menu_categories_section` refuse un chapitre sur une sous-catégorie (`MENU_CATEGORY_SECTION_ON_CHILD`, P0426), traduit par `describeMenuError`. Côté carte publique, le chapitre est un accordéon plus marqué qui liste ses catégories en lignes repliables.
...
**Carte publique et QR codes (septembre 2026)** : la carte que voit le client est le projet frère **`royaume-menus`** (`~/Documents/dev/royaume-menus`, dépôt privé `ip-dev-lu/royaume-menus`, ex-`menu-ripaille`), déployé sur Vercel sous **`menus.auxparaiges.fr`**, une carte par slug d'établissement (`/la-ripaille`, `/delirium-cafe-strasbourg`). Il ne lit que `get_public_menu*` avec la clé anon. L'ancien domaine `la-ripaille.auxparaiges.fr` reste rattaché à ce projet et renvoie en 307 (QR codes déjà imprimés), via son `next.config.ts`. **Les QR codes des tables encodent les liens courts** `redirects.auxparaiges.fr/menu-<nom court>` (`menu-ripaille`, `menu-delirium`, `menu-aux-paraiges`, `menu-troubadour`, `menu-garage`, `menu-grange`, `menu-chapelle`), gérés dans `/links` : un lien par établissement, **actif seulement quand sa carte est saisie** (inactif = repli `auxparaiges.fr`). Activer le lien fait partie de la mise en service d'une carte, au même titre que la saisie des catégories.

**Saisie d'une carte par migration (Garage des Paraiges, migration 104, septembre 2026)** : première des cinq cartes vides, la carte tient en JSON inline dans un `DO` (la fonction de chargement de la 099 ne se justifiait que pour 126 Ko). Conventions à reprendre pour les quatre suivantes (Aux Paraiges, Troubadour, Grange, Chapelle) : happy hour = plage sur l'établissement + seconde variante `is_happy_hour` sur chaque produit concerné ; « disponible sans alcool » = colonne `precision` (mention inline à côté du titre) ; format « 4 cl » posé en variante **et** rappelé dans la description de la catégorie, car la liste publique n'affiche pas le libellé d'un item à variante unique ; prix absent de la carte imprimée = `price NULL` (affiché « — »), jamais inventé, à compléter depuis `/menus` ; soft dont la marque fait partie du nom = nouveau produit de catalogue (précédent Ice Tea / Fuze Tea) ; mentions légales = catégorie racine sans item, description seule. **Un produit de catalogue créé pour une nouvelle carte n'est pas activé sur les cartes existantes** des autres établissements (consigne du 08/09/2026) : les quatre nouveaux softs ne sont à la carte que du Garage, Delirium et La Ripaille ne bougent pas. Le lien court reste inactif après la saisie, l'activer est un acte séparé.

**Édition d'une carte pensée pour le téléphone (septembre 2026)** : `/menus/[id]` se pilote au pouce, en service, sans perdre l'usage desktop. Le rendu vit dans `menus/[id]/_components/` : `category-nav.tsx` (barre **collante** en haut de la page : **champ de recherche** puis puces, une par catégorie racine + « Hors carte », tap = défilement vers la section, puce active suivie par IntersectionObserver dont la racine est `<main>` ; expose `SECTION_SCROLL_MARGIN` à poser sur chaque section et la variable CSS `--menu-nav-h`. ⚠️ Le `sticky` s'arrête au bord du **contenu** de `<main>`, sous son padding : la barre porte `-top-4 md:-top-6` pour se recoller au header, sinon un jour de 16 px laisse voir la page défiler entre les deux), `_lib/search.ts` (**recherche** : sans casse ni accents, ligatures dépliées, chaque mot tapé doit se retrouver dans la fiche, dans n'importe quel ordre ; une faute de frappe est tolérée à partir de 4 lettres (distance d'édition 1) ; l'index d'un produit couvre nom, descriptif, précision, allergènes, famille, source, **chemin de catégorie** (« pression blonde »), libellés de formats, **prix** avec virgule ou point, et des mots-clés d'état sans interface dédiée : « rupture », « hh » / « happy hour », « coup de cœur », « hors carte » ; une catégorie dont le titre répond à tous les mots est gardée entière, ce qui garde trouvables les blocs de texte sans produit ; `findHighlightRanges` surligne les correspondances exactes dans le nom via `<Highlighted>` en gardant la correspondance des index malgré la normalisation ; la saisie passe par `useDeferredValue`, pas de debounce), `category-section.tsx` (un bloc par catégorie racine, sous-catégories **à plat** dessous, plus de cartes imbriquées qui mangent la largeur ; les catégories vides sont rendues, c'est depuis leur en-tête qu'on y ajoute un produit), `menu-item-row.tsx` (toute la ligne ouvre la fiche, l'interrupteur de disponibilité reste inline car c'est le geste le plus fréquent en service ; les raccourcis étoile / crayon / œil barré ne s'affichent qu'à partir de `md`), `menu-item-sheet.tsx` (**fiche d'actions rapides** d'un produit : interrupteurs Disponible et Coup de cœur, **modification des prix sans ouvrir le formulaire** via `replaceMenuItemVariants`, changement de catégorie ou placement sur la carte via `moveMenuItem`, retrait de la carte, lien vers la fiche complète), `category-actions-sheet.tsx` (ouverte par l'en-tête d'une catégorie : ajouter un produit ici, le formulaire s'ouvrant pré-rempli via `?category=<id>`, modifier, masquer / afficher, supprimer), `menu-action-bar.tsx` (les deux créations dans une barre **fixée en bas** sous `md`, la page réserve `pb-24` ; l'en-tête garde ses boutons au-delà). Les interrupteurs de la liste sont **optimistes** (`setQueryData` puis invalidation, retour arrière par refetch en cas d'erreur), le reste attend le refetch : `runItemAction` renvoie un booléen et la feuille ne remet son formulaire de prix à zéro qu'en cas de succès. Le formulaire produit (`_form/MenuItemForm.tsx`) empile chaque format en encart de deux lignes sous `sm` (`sm:contents` aplatit en une ligne au-delà), resserre les marges des cartes et colle ses boutons en bas de l'écran. Le prix a un seul helper, `_lib/price.ts` (`priceInputSchema`, `parsePriceInput`, `formatPriceInput`, `formatPrice`), partagé par le formulaire et la fiche. Trois réglages **globaux** sont venus avec : `<Input>` / `<Textarea>` / `<SelectTrigger>` en `text-base md:text-sm` (sous 16 px, iOS zoome à la focalisation), `DashboardShell` en `h-dvh` au lieu de `h-screen` (le bas de `<main>` passait sous la barre d'outils de Safari), et `<BottomSheet>` (cf. *Composants UI partagés*). ⚠️ **Piège Zod 4 corrigé au passage** : `.partial()` conserve les `.default()`, donc `menuItemUpdateSchema.parse({ category_id })` renvoyait aussi `variants: []` et `is_featured: false` : un update partiel aurait **effacé les formats et retiré le coup de cœur** (même chose pour `position: 0` côté catégories). Les schémas de mise à jour dérivent désormais d'un noyau sans défaut (`menuItemCoreSchema`, `menuCategoryCoreSchema`) : ne jamais y remettre de `.default()`, les défauts n'appartiennent qu'aux schémas de création.
...
- **`profiles.is_super_admin`** (seed : `direction@ipdev.lu`) : un super admin peut activer/désactiver l'accès **par page** aux autres admins via l'onglet **Administrateurs** de `/settings` (accordéon `admin-access-section.tsx`, un admin par panneau, en-tête = nom + e-mail + compteur d’accès, les interrupteurs ne sont montés que dans le panneau déplié - composant `src/components/ui/accordion.tsx` ajouté en septembre 2026 ; onglet visible super admin uniquement, deep-link `?tab=admins` — entrée sidebar « Gestion des admins » dans Système, `superAdminOnly: true` sur le `NavItem`). Colonne **immuable côté client** (trigger anti-escalade `trg_protect_is_super_admin`, promotion uniquement en SQL service_role/postgres).
...
- **Feature-gating en RLS (migration 070)** : le blocage par fonctionnalité n'est plus seulement middleware. Helper `admin_has_feature(feature_key text)` (`SECURITY DEFINER`, STABLE) = super-admin **OU** admin dont `feature_key` n'est pas dans `admin_disabled_features`. Les écritures (INSERT/UPDATE/DELETE) de `quests`, `quest_iterations`, `quest_periods`, `quests_establishments` exigent `admin_has_feature('quests')` — un admin restreint est bloqué **même via appel REST direct** (le form admin passant par la clé anon + RLS). Les lectures restent inchangées. Pour étendre à une autre feature : reprendre le même pattern sur ses tables de config.
- **Feature-gating généralisé (migration 114, septembre 2026)** : **toutes les écritures** du dashboard sont désormais gardées en base, plus seulement quests / reports / menus. Policies `<table>_feature_insert|update|delete` sur `admin_has_feature` / `admin_has_any_feature(VARIADIC)` (plusieurs clés quand deux pages écrivent la même table : `badge_types` = achievements + rewards, `coupons` = coupons + cashback-gains) ; RPC `SECURITY DEFINER` sur `assert_admin_feature('<clé>', …)` à la place de `assert_admin()` (`create_manual_coupon`, `distribute_period_rewards_v2`, RPC de saison, `distribute_quest_reward(s)`, `admin_delete_receipt`, `admin_reset_identity_photo_cooldown`, branche admin de `gdpr_*`). Cartographie complète dans `docs/docs/supabase/policies/README.md` (section *Feature-gating*). **Les lectures restent ouvertes** à tout admin (l'accueil `/` affiche des chiffres globaux) : gater la lecture est un chantier séparé. Le middleware applique aussi la feature aux routes `/api/<page>/…` (préfixe `/api` retiré avant `resolveFeatureKey`) et la route vidéo revérifie `admin_has_feature('reports')`. ⚠️ Nouvelle table de config = nouvelle policy gatée, sinon elle retombe en `role='admin'` simple. **Refus audibles (migration 115 + `src/lib/supabase/access-errors.ts`)** : les policies d'UPDATE gatées testent la feature dans `WITH CHECK` seul (USING = rôle admin), donc un UPDATE hors feature lève `42501` au lieu de toucher zéro ligne ; INSERT et RPC lèvent aussi `42501` (`FEATURE_DISABLED:`). Seul DELETE reste silencieux : les suppressions unitaires des services font `.delete().eq(…).select("id")` puis `assertWriteTouched(data, "ce palier")` (ne pas l'appliquer aux patterns « delete all puis insert », où zéro ligne est légitime). Le client navigateur (`createClient`, option `global.fetch`) traduit tout corps d'erreur `42501` en message français, corps d'origine dans `details` : les toasts qui affichent `error.message` sont lisibles sans rien changer. Les policies `menu_*` gardent leur USING scopé (le périmètre dépend de la ligne) et `menuService` son `assertTouched` local.
...
- **Pages** : `/reports` (liste + interrupteur d'activation), `/reports/[key]` (destinataires à cocher, envoi manuel, envoi de test, prévisualisation, historique des runs) et `/reports/recipients` (annuaire). Service `emailReportService.ts`, keys `emailReportKeys`, Zod `emailReport.schema.ts`.
- **Destinataires définis une fois, cochés par rapport (migration 112, septembre 2026)** : l'annuaire `email_report_contacts` porte l'adresse, le libellé et l'interrupteur `is_active` ; `email_report_recipients` n'est plus qu'un pivot (`report_id`, `contact_id`). La carte Destinataires de `/reports/[key]` liste **tout l'annuaire en cases à cocher** (optimiste, `setReportRecipient` = upsert / delete) et son formulaire « Ajouter et cocher » crée le contact **et** l'abonne ; sur un `23505` (adresse déjà connue) il coche l'existant au lieu d'échouer. Suspendre un contact (`/reports/recipients`) l'exclut de **tous** les envois sans décocher ses rapports ; le supprimer cascade sur le pivot (ConfirmDialog le rappelle). `countActiveRecipients` = cochés **et** actifs, c'est ce chiffre que la liste et le bouton d'activation utilisent. L'Edge Function lit le pivot joint à l'annuaire (`email_report_contacts!inner`, `is_active = true`). Schémas Zod `contactSchema` / `contactUpdateSchema` (ce dernier dérive d'un noyau sans `.default()`, piège Zod 4).
...
- **Envoi manuel : la période se choisit dans une liste**, plus de saisie libre d'identifiant (septembre 2026). `reports/_lib/period-options.ts` (`buildReportPeriodOptions`) génère localement la période en cours, les 12 précédentes, et la suivante pour un rapport d'annonce (`period_scope = current`) ; la période visée par défaut est signalée dans la liste et l'option « Par défaut » laisse l'Edge Function la résoudre. Semaines ISO via `getPeriodIdentifier` (jamais de calcul local), ancre = premier jour de la période courante pour éviter le débordement d'un 31 décalé d'un mois. `periodIdentifierSchema` reste dans `emailReport.schema.ts` mais n'est plus branché sur l'UI.
...
- **Vidéo en pièce jointe** (migration 082) : un rapport de classement peut embarquer une vidéo générée depuis un template HyperFrames. Carte « Vidéo » dans `/reports/[key]` (`_components/video-card.tsx`) : interrupteur, aperçu `<video>` par URL signée, historique des rendus, relance. **Chaque rendu `ready` de l'historique a son bouton « Regarder »** (septembre 2026) : la purge à 7 jours laisse souvent plusieurs vidéos lisibles en même temps, et « Regarder la dernière vidéo » vise la plus récente **encore `ready`**, pas la dernière ligne (qui peut être en cours ou en erreur). La liste `/reports` mentionne l'état de la vidéo sur chaque carte (`latest_video` dans `EmailReportWithStats`, calculé par `getEmailReports` : la vidéo `ready` la plus récente prime sur un rendu plus frais mais inexploitable ; rien n'est affiché si l'option est désactivée et qu'aucun fichier ne subsiste). **Aucune notion HyperFrames n'est exposée à l'admin.** Opt-in par rapport via `email_reports.options.video === true` (JSONB existant, pas de migration pour l'activer). Le rendu vit dans le projet Vercel séparé `royaume-video-renderer` et s'appelle via la route serveur `src/app/api/reports/[key]/video-render/route.ts` (le bearer partagé ne doit jamais descendre dans le navigateur ; env `VIDEO_RENDER_URL` / `VIDEO_RENDER_KEY`). État lu dans `report_video_renders`, fichier dans le bucket privé `report-videos` purgé à 7 jours. Statuts `queued`/`rendering`/`ready`/`expired` ajoutés au registre central `status-badge.tsx`. Détail complet : `CLAUDE.md` du workspace, section « Vidéo en pièce jointe ».
