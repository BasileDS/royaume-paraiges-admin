...
- **Restitution du journal de distribution non faite.** `reward_distribution_runs` (migration 083) est alimentée mais n'a ni page admin ni alerte. Prévu : section sur `/rewards` (DataTable + `<StatusBadge>`, le vocabulaire de statut est déjà celui du registre) et rapport e-mail d'alerte quand une distribution ne récompense personne — ce second point touche `email_reports` / `get_email_report_payload`, à coordonner avec la session qui travaille dessus.
