...
- **Reste à faire : page admin du journal.** `reward_distribution_runs` n'est consultable qu'en SQL. Prévu : section sur `/rewards` en `<DataTable>` — le vocabulaire de statut (`success` / `partial` / `error` / `skipped`) est déjà celui du registre `<StatusBadge>`.
